...
// Имя изображения
(() => {
  const emptyLabelText = "Выберите изображение";
  const fileInput = document.getElementById("file-uploader");
  const label = document.getElementById("file-name");
  const clearButton = document.getElementById("file-clear");

  if (!label || !fileInput || !clearButton) {
    return;
  }
  const fileChangeHandler = () => {
    const file = fileInput.files[0];
    if (!file) {
      label.innerText = emptyLabelText;
    } else {
  ...
  clearButton.addEventListener("click", () => fileClearHandler());
})();
